...
- Releases: before cutting one, read `docs/RELEASING.md`. If it’s missing, search for the best-fit release checklist and alert the user. GitHub releases must be titled `<Project> <version>` (never just the version) and mirror the curated changelog bullets verbatim.
